...
class ReservationFetcher:
    ...
    def get_reservation_summary(room_number: str, password: str, building_id: str, connection=None) -> dict:
        ...
        try:
            # 1. 現在の予約を取得
            current_reservation = ReservationFetcher.get_reservation_date(room_number, password, building_id, connection=connection)
            if "error" in current_reservation:
                return current_reservation
            
            # 2. 予約状況を取得
            status_info = ReservationFetcher.get_reservation_status(room_number, password, building_id, connection=connection)
            if "error" in status_info:
                return status_info
            
            # 3. 今後の予約を取得
            upcoming_reservations = ReservationFetcher.get_upcoming_reservations(room_number, password, building_id, connection=connection)
...
def get_reservation_date(room_number: str, password: str, building_id: str, connection=None) -> dict:
    ...
    return fetcher.get_reservation_date(room_number, password, building_id, connection=connection)
...
def get_reservation_history(room_number: str, password: str, building_id: str, 
    ...
    """予約履歴を取得（外部呼び出し用）"""
    fetcher = ReservationFetcher()
    return fetcher.get_reservation_history(room_number, password, building_id, limit, connection=connection)


def get_reservation_status(room_number: str, password: str, building_id: str, connection=None) -> dict:
    """予約状況を取得（外部呼び出し用）"""
    fetcher = ReservationFetcher()
    return fetcher.get_reservation_status(room_number, password, building_id, connection=connection)
...
def get_upcoming_reservations(room_number: str, password: str, building_id: str, 
    ...
    """今後の予約を取得（外部呼び出し用）"""
    fetcher = ReservationFetcher()
    return fetcher.get_upcoming_reservations(room_number, password, building_id, days_ahead, connection=connection)


def get_reservation_summary(room_number: str, password: str, building_id: str, connection=None) -> dict:
    """予約サマリーを取得（外部呼び出し用）"""
    fetcher = ReservationFetcher()
    return fetcher.get_reservation_summary(room_number, password, building_id, connection=connection)
